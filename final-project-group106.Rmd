...
#kable(t(head(data[data$infant.deaths > boxplot(data$infant.deaths)$stats[5], ])))
...
#summary(model.additive.step.aic)
par(mfrow = c(2, 2))
...
#summary(model.additive.step.bic)
par(mfrow = c(2, 2))
...
#summary(model.interaction.step.aic)
...
model_selected_outliers=lm(formula = Life.expectancy ~  Adult.Mortality  +
    ...
    Schooling  + Adult.Mortality:HIV.AIDS + I(HIV.AIDS^2), data = data.train, subset=abs(rstandard(model_selected_poly_interactions_aic))<2)
...
model_5 = lm(Life.expectancy ~ (Year + Status + Adult.Mortality + infant.deaths + Alcohol + percentage.expenditure + BMI + Polio + Total.expenditure
                                + Diphtheria + HIV.AIDS + thinness..1.19.years + Income.composition.of.resources + Schooling) ^ 2, data = data.train)
par(mfrow=c(2,2))
plot(model_5)

show_metrics(model_5)

```

```{r}
model_8 = lm(Life.expectancy ~ Adult.Mortality + BMI + Diphtheria + thinness..1.19.years + Income.composition.of.resources + Schooling +  Year:Income.composition.of.resources + Year:Schooling + Status:Adult.Mortality + Status:thinness..1.19.years + Adult.Mortality:Schooling + infant.deaths:Income.composition.of.resources + infant.deaths:Schooling + Alcohol:Total.expenditure + Alcohol:HIV.AIDS + Alcohol:Income.composition.of.resources + BMI:Diphtheria + BMI:Income.composition.of.resources + BMI:Schooling + Total.expenditure:Income.composition.of.resources + I(Adult.Mortality^2) + I(BMI^2) + I(Diphtheria^2) + I(thinness..1.19.years^2) + I(Income.composition.of.resources^2) + I(Schooling^2), data = data.train)
length(coef(model_8))
```
...
```{r}
model_11 = lm(Life.expectancy ~ (Adult.Mortality + BMI + Diphtheria + thinness..1.19.years + Income.composition.of.resources + Schooling) ^ 2 + I(Adult.Mortality^2) + I(BMI^2) + I(Diphtheria^2) + I(thinness..1.19.years^2) + I(Income.composition.of.resources^2) + I(Schooling^2), data = data.train)
...
```{r}
# This is the model that Mayank built
formula = lm(Life.expectancy ~ Adult.Mortality + infant.deaths + Polio + HIV.AIDS + thinness.5.9.years + Income.composition.of.resources + Schooling + Adult.Mortality:HIV.AIDS + I(HIV.AIDS^2), data = data_trn)
```
...
par(mfrow=c(1,2))
boxcox(formula, plotit = TRUE)
boxcox(formula, plotit = TRUE, lambda = seq(0.5, 1.5, by = 0.1))
```

Explore the relationship between Life.expectancy and each predictor.
```{r}
plot(Life.expectancy ~ Adult.Mortality, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs Adult.Mortality")

plot(Life.expectancy ~ infant.deaths, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs infant.deaths")

plot(Life.expectancy ~ Polio, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs Polio")

plot(Life.expectancy ~ HIV.AIDS, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs HIV.AIDS")

plot(Life.expectancy ~ thinness.5.9.years, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs thinness.5.9.years")

plot(Life.expectancy ~ Income.composition.of.resources, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs Income.composition.of.resources")

plot(Life.expectancy ~ Schooling, data = data_trn, col = "grey", pch = 20, cex = 1.5,
main = "Life.expectancy vs Schooling")
```

```{r}
lm_le_am = lm(Life.expectancy ~ Adult.Mortality, data = data_trn)
show_metrics(lm_le_am)
lm_le_am_log = lm(Life.expectancy ~ log(Adult.Mortality), data = data_trn)
show_metrics(lm_le_am_log)
# bp.test result is significantly improved with log transformation of Adult.Mortality.
lm_le_am_poly = lm(Life.expectancy ~ Adult.Mortality + I(Adult.Mortality ^ 2), data = data_trn)
...
```{r}
lm_le_id = lm(Life.expectancy ~ infant.deaths, data = data_trn)
show_metrics(lm_le_id)
# lm_le_id_log = lm(Life.expectancy ~ log(infant.deaths), data = data_trn)
# couldn't perform log transformation because of O values.

lm_le_id_poly = lm(Life.expectancy ~ infant.deaths + I(infant.deaths ^ 2), data = data_trn)
show_metrics(lm_le_id_poly)
# Polynomial transformation of infant.deaths doesn't improve.
```

```{r}
lm_le_po = lm(Life.expectancy ~ Polio, data = data_trn)
show_metrics(lm_le_po)
lm_le_po_log = lm(Life.expectancy ~ log(Polio), data = data_trn)
show_metrics(lm_le_po_log)
# bp.test result is improved with log transformation of Polio.
lm_le_po_poly = lm(Life.expectancy ~ Polio + I(Polio ^ 2), data = data_trn)
...
```{r}
lm_le_hiv = lm(Life.expectancy ~ HIV.AIDS, data = data_trn)
show_metrics(lm_le_hiv)
lm_le_hiv_log = lm(Life.expectancy ~ log(HIV.AIDS), data = data_trn)
show_metrics(lm_le_hiv_log)
# bp.test result is significantly improved with log transformation of HIV.AIDS.
lm_le_hiv_poly = lm(Life.expectancy ~ HIV.AIDS + I(HIV.AIDS ^ 2), data = data_trn)
show_metrics(lm_le_hiv_poly)
# Polynomial transformation also helps.
lm_le_hiv_log_poly = lm(Life.expectancy ~ log(HIV.AIDS) + I(HIV.AIDS ^ 2), data = data_trn) # doesn't further improve
show_metrics(lm_le_hiv_log_poly)
```

```{r}
lm_le_thn = lm(Life.expectancy ~ thinness.5.9.years, data = data_trn)
show_metrics(lm_le_thn)
lm_le_thn_log = lm(Life.expectancy ~ log(thinness.5.9.years), data = data_trn)
show_metrics(lm_le_thn_log)
# bp.test result is significantly improved with log transformation.
lm_le_thn_poly = lm(Life.expectancy ~ thinness.5.9.years + I(thinness.5.9.years ^ 2), data = data_trn)
...
```{r}
lm_le_icr = lm(Life.expectancy ~ Income.composition.of.resources, data = data_trn)
show_metrics(lm_le_icr)
# lm_le_icr_log = lm(Life.expectancy ~ log(Income.composition.of.resources), data = data_trn) couldn't perform log tranformation due to 0 values.

# bp.test result is significantly improved with log transformation.
lm_le_icr_poly = lm(Life.expectancy ~ Income.composition.of.resources + I(Income.composition.of.resources ^ 2), data = data_trn)
show_metrics(lm_le_icr_poly)
# Polynomial transformation helps a little.
```

```{r}
lm_le_sch = lm(Life.expectancy ~ Schooling, data = data_trn)
show_metrics(lm_le_sch)
lm_le_sch_log = lm(Life.expectancy ~ log(Schooling), data = data_trn)
show_metrics(lm_le_sch_log)
# log transformation doesn't work.
lm_le_sch_poly = lm(Life.expectancy ~ Schooling + I(Schooling ^ 2), data = data_trn)
...
model_trans = lm(Life.expectancy ~ log(Adult.Mortality) + infant.deaths + Polio  + I(Polio ^ 2) + HIV.AIDS + I(HIV.AIDS^2) + log(thinness.5.9.years) + Income.composition.of.resources + Schooling + Adult.Mortality:HIV.AIDS, data = data_trn)
show_metrics(formula)
